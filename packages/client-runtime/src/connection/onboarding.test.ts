import { AuthStandardClientScopes, EnvironmentId } from "@vipercode/contracts";
...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/viper/environment")) {
  ...
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(calls.map((call) => call.url)).toEqual([
        "https://remote.example.test/.well-known/viper/environment",
      ...
      expect(tokenParams.get("client_label")).toBe("Viper Code Test");
    }),
  );
